...
#[repr(u8)]
pub enum MsgType {
    /// → start index u16  ← start u16, next u16, repeated definitions.
    GetParams = 1,
...
mod tests {
    ...
    fn known_answer_discovery_requests() {
        let mut buf = [0u8; 16];
        let n = encode(&mut buf, MsgType::GetParams as u8, 1, &[0, 0]).unwrap();
        assert_eq!(
            &buf[..n],
            &[0x48, 0x4c, 0x01, 0x01, 0x02, 0x00, 0x00, 0x00, 0x89, 0x0c]
        );
    ...
    fn known_answer_v3_frames() {
        ...
        let status = [
            0x03, 0x11, 0x00, 0x0d, 0x00, 0x00, 0xfa, 0x45, 0x10, 0xa4, 0x00, 0x00,
        ];
        let n = encode(&mut buf, MsgType::Status as u8, 1, &status).unwrap();
        assert_eq!(
            &buf[..n],
            &[
                0x48, 0x4c, 0x0a, 0x01, 0x0c, 0x00, 0x03, 0x11, 0x00, 0x0d, 0x00, 0x00, 0xfa, 0x45,
                0x10, 0xa4, 0x00, 0x00, 0x76, 0x0a
            ]
        );
    }
