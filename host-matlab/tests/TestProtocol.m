...
classdef TestProtocol < matlab.unittest.TestCase
    methods (Test)
        function crcKnownAnswers(testCase)
            testCase.verifyEqual(helicdaq.Protocol.MCU_REBOOT_CONFIRMATION, ...
                uint32(hex2dec('52454254')));
        ...
        function frameKnownAnswers(testCase)
            testCase.verifyEqual( ...
                ...
                helicdaq.Protocol.encodeParamPageRequest(0)), ...
                TestProtocol.hexBytes('48 4C 01 01 02 00 00 00 89 0C'));
            testCase.verifyEqual( ...
        ...
        function discoveryEntries(testCase)
            ...
            pagePayload = uint8([7, 0, 8, 0, uint8('freq'), 0, ...
                uint8('f'), 1, 0, 1]);
            [startIndex, nextIndex, page] = ...
                helicdaq.Protocol.decodeParamPage(pagePayload);
            testCase.verifyEqual(startIndex, uint16(7));
            testCase.verifyEqual(nextIndex, uint16(8));
            testCase.verifyEqual(page.Index, uint16(7));
            testCase.verifyEqual(page.Name, "freq");
            testCase.verifyError(@() helicdaq.Protocol.decodeParamPage(uint8([0, 0, 0])), ...
                'helicdaq:ProtocolError');
            testCase.verifyError(@() helicdaq.Protocol.decodeParamPage(uint8([2, 0, 1, 0])), ...
                'helicdaq:ProtocolError');
        ...
        function parameterTypesRoundTrip(testCase)
            ...
            encoded = helicdaq.Protocol.packParameter('c', 8, "magneto");
            testCase.verifyEqual( ...
                helicdaq.Protocol.unpackParameter('c', 8, encoded), "magneto");
        end
        ...
        function beaconKnownAnswer(testCase)
            ...
            testCase.verifyEqual(decoded.Experiment, "magnetoelastic");
