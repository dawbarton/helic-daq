...
class ControlLinkLost(RuntimeError):
    """The device stopped answering the control channel part-way through a run.

    Distinct from an acceptance failure: nothing further can be measured, so
    the run ends where it is and reports what it had.
    """
    ...
    while time.monotonic() - started < seconds:
        for write in profile.quiet:
            ...
            try:
                ...
                # The device stopped answering entirely. A regression should
                # report that as a result, not raise a traceback out of the
                # tool, so the phase counters gathered so far still reach the
...
def main() -> int:
    ...
    try:
        measure(args, profile, regression, host, result)
    except ControlLinkLost as lost:
        # Nothing after this point is measurable, so report rather than raise.
        result["control_link_lost"] = str(lost)
        errors = [f"control link lost: {lost}"]
        errors.extend(acceptance_errors(result, regression))
